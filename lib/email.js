...
async function send(to, subject, html) {
  ...
  const from = s.fromEmail || 'hello@499web.co';
  try {
    const res = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { Authorization: `Bearer ${s.resendApiKey}`, 'content-type': 'application/json' },
      body: JSON.stringify({ from: `499 Web Co. <${from}>`, to: [to], subject, html })
    });
...
function shell(title, body) {
  return `<div style="font-family:-apple-system,Segoe UI,Inter,sans-serif;max-width:560px;margin:0 auto;background:#0c0e15;color:#eaf0ff;border-radius:16px;overflow:hidden;border:1px solid #222840">
    <div style="background:linear-gradient(100deg,#1aa6ff,#ff2e7e);padding:18px 26px;font-weight:800;font-size:20px;color:#04121f">499 WEB CO.</div>
    <div style="padding:26px">
      <h1 style="font-size:22px;margin:0 0 12px">${title}</h1>
      ${body}
      <p style="color:#9aa3bd;font-size:13px;margin-top:28px;border-top:1px solid #222840;padding-top:16px">499 Web Co. · Great websites for $499 · Gwinnett County &amp; Metro Atlanta</p>
    </div></div>`;
}
const btn = (href, label) => `<a href="${href}" style="display:inline-block;background:linear-gradient(100deg,#5fd8ff,#ff2e7e);color:#04121f;font-weight:800;text-decoration:none;padding:13px 24px;border-radius:10px;margin:8px 0">${label}</a>`;

export function prospectConfirmation(to, name) {
  return send(to, 'We got your request — your $499 site is on the way',
    shell('Thanks' + (name ? `, ${name}` : '') + '! 🏁', `
      <p>We received your request and the 499 crew is on it. Here's what happens next:</p>
      <ol style="color:#cfd6ee;line-height:1.7">
        ...
        <li>You approve it, pay the one flat $499, and we launch it.</li>
      </ol>
...
export function teamLeadNotice(to, lead) {
  return send(to, `New $499 lead: ${lead.name || 'Unknown business'}`,
...
export function previewReady(to, name, reviewUrl) {
  return send(to, 'Your new website is ready to preview 👀',
    shell('Take a look' + (name ? `, ${name}` : '') + '!', `
      ...
      <p style="color:#9aa3bd;font-size:13px">One flat price — $499, one time. No payment until you approve. This link is private to you.</p>`));
}
...
export function siteLive(to, name, liveUrl, domain, records) {
  ...
  return send(to, '🎉 Your website is live!',
    shell('You\'re live' + (name ? `, ${name}` : '') + '! 🏁', `
      ...
      <table style="border-collapse:collapse;font-size:13px;color:#eaf0ff;margin:10px 0"><tr><td style="padding:6px 10px;color:#9aa3bd">Type</td><td style="padding:6px 10px;color:#9aa3bd">Name</td><td style="padding:6px 10px;color:#9aa3bd">Value</td></tr>${rows}</table>` : ''}
      <p>Thanks for choosing 499 Web Co. — reply anytime if you need a change.</p>`));
}
