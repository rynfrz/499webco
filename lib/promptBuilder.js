...
- LOGO: if the brief says a brand logo is provided, use the EXACT literal token {{LOGO_SRC}} as the logo image src in the header (and footer if fitting) and as the favicon (<link rel="icon" href="{{LOGO_SRC}}">). Never invent or draw a different logo, and don't leave the token anywhere a logo isn't intended. If NO logo is provided, design a tasteful wordmark from the business name instead and omit the token entirely.
...
export function buildWebsitePrompt(project) {
  ...
  if (b.photos?.length) {
    lines.push(`REAL PHOTOS of this business (${b.photos.length}) — these are safe, hosted URLs. Use them PROMINENTLY and generously as real <img> tags throughout: the hero, an image gallery/grid, the about section, and to accent service cards. Prefer these over any gradient/stock treatment. Use object-fit:cover, responsive sizing, lazy loading, and descriptive alt text. Distribute them across sections — do not cram them into one place.`);
    b.photos.forEach(u => lines.push(`- ${u}`));
  }
  ...
  if (b.logo) {
    lines.push('');
    lines.push(`## BRAND LOGO (provided)`);
    lines.push(`A real brand logo is provided. Use the EXACT token {{LOGO_SRC}} as the logo <img> src in the header (≈36–48px tall) and, if fitting, the footer; set alt="${b.name} logo". Also add the favicon: <link rel="icon" href="{{LOGO_SRC}}">. Do not invent a different logo or replace it with text.`);
    if (b.logoColors?.length) {
      lines.push(`The logo's brand colors are: ${b.logoColors.join(', ')}. Harmonize the whole palette with these — favor them for the primary/accent and gradients so the site feels on-brand with the logo, while keeping the committed palette's structure and contrast.`);
    }
  }

  if (q.designNotes && q.designNotes.trim()) {
    lines.push('');
    lines.push(`## DIRECT DESIGN DIRECTION FROM THE DESIGNER (highest priority — follow closely)`);
    lines.push(q.designNotes.trim());
  }
...
Structure:
1. Header: "Website Audit — ${b.name}" with date ${new Date().toLocaleDateString()} and "Prepared by 499web.co".
